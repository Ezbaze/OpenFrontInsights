<script lang="ts">
	...
	const makeSortingFn =
		() => (rowA: { getValue: (id: string) => unknown }, rowB: { getValue: (id: string) => unknown }, columnId: string) =>
	...
	$effect(() => {
		columnVisibility = {
			game: clanSessions.some((session) => {
				const value = rawValues.game(session);
				return value !== null && value !== undefined && value !== "";
			}),
			start: clanSessions.some((session) => {
				const value = rawValues.start(session);
				return value !== null && value !== undefined && value !== "";
			}),
			end: clanSessions.some((session) => {
				const value = rawValues.end(session);
				return value !== null && value !== undefined && value !== "";
			}),
			result: clanSessions.some((session) => {
				const value = rawValues.result(session);
				return value !== null && value !== undefined && value !== "";
			}),
			teams: clanSessions.some((session) => {
				const value = rawValues.teams(session);
				return value !== null && value !== undefined && value !== "";
			}),
			clanPlayers: clanSessions.some((session) => {
				const value = rawValues.clanPlayers(session);
				return value !== null && value !== undefined && value !== "";
			}),
			players: clanSessions.some((session) => {
				const value = rawValues.players(session);
				return value !== null && value !== undefined && value !== "";
			}),
		};
	...
	let sessionsVirtualStart = $state(0);
	let sessionsVirtualEnd = $state(0);
	...
	$effect(() => {
		...
		const nextStart = Math.max(0, Math.floor(effectiveScrollTop / sessionRowHeight) - sessionOverscan);
		const nextEnd = Math.min(rows.length, nextStart + visibleCount + sessionOverscan * 2);
		sessionsVirtualStart = nextStart;
		sessionsVirtualEnd = nextEnd;
...
<Dialog.Root bind:open={dialogOpen}>
	<Dialog.Content class="!w-[92vw] !max-w-[1280px] sm:!max-w-[1280px] max-h-[90vh] p-0">
									...
									<Table.Row>
										{#each headerGroup.headers as header (header.id)}
											...
											<Table.Head
												...
												class={[
													"sticky top-0 z-20 bg-background border-l border-border/40 first:border-l-0",
